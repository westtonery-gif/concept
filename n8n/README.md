...
- **2026-09-21 — the clipping department's local toolchain is live and verified (ADR-0063,
  ADR-0064).** `brew install ffmpeg` gave **9.0.2** with `ffmpeg` and `ffprobe` at
  `/opt/homebrew/bin` (Homebrew's PATH line is in `~/.zprofile`, so an interactive shell finds
  them; a non-interactive one does not, and the tests that need them skip there). **That build
  cannot burn captions:** `ffmpeg -h filter=subtitles` and `-h filter=drawtext` both answer
  `Unknown filter`, and neither its `configuration:` line nor `brew deps ffmpeg` lists `libass` or
  `libfreetype` — hence ADR-0063. `brew install whisper.cpp` gave **1.9.4** (MIT), and the operator
  downloaded `ggml-large-v3-turbo.bin` (1.62 GB) to `~/whisper-models/`; the formula ships no
  weights on purpose.
  **Verified on real material, not only on fixtures:** `scdet` finds a black→white cut at 2023 ms
  in a concatenated 4-second file (the join shifts by a frame or two, which is why the acceptance
  asserts "one cut, near 2.0 s" rather than an exact figure). And with a Russian utterance
  generated by macOS `say`, `LocalFootageIndex` returned
  `0–4540 ms · 'Первая реплика героя, а это уже вторая сцена, и говорит другой персонаж.'` against
  a 4680 ms clip — verbatim apart from one comma, with usable timings, and no scenes invented on a
  static frame. **Noted for tuning:** whisper returned the whole utterance as **one** segment. On a
  real episode that means coarse pause information for boundary nudging and long captions if
  burn-in ever returns; `whisper-cli`'s `--max-len` is the knob, and the first real episode decides
  it.
