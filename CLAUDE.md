...
28. ~~**Skip the opening titles and end credits**~~ — done (ADR-0074, 2026-09-22). Second episode `~/episodes/rick-morty-2.mp4` (S1E2, found in `~/Downloads` as `video_a40fd6a3…_20260922134810.mp4`, cloned in; the file the maintainer had renamed `rick-morty-2.mp4` in `clips/rerun-24/` was clip 01 and was renamed back). `fpcalc` (chromaprint 1.6.1) raw fingerprints of both: opening theme ep1 127.3–159.4 s / ep2 0–30.9 s, end theme ep1 1295.4–1314.0 s / ep2 1246.6–1280.2 s (ep2 has ~38 s after it — a post-credits scene, kept). `IndexedFootage.skips` (`SkipZone`), found by `LocalFootageIndex` against up to four sibling files in the same folder (shared ≥ 15 s within the first/last quarter; pure matcher `infrastructure/recurring_audio.py`), recorded in `footage-index@v1`, and `plan_clips` plans each stretch between zones separately (slivers < 5 s dropped). Re-planning ep1 with them: no clip covers the titles. **Left:** the dub credit «Перевел Сайрус…» spoken at the theme's edge still enters clip 3's transcript (a span straddling the zone edge); the 7 s tail after ep1's credits is whisper's classic silence hallucination «Редактор субтитров…», which QA's no-content rule should fail; the manual fallback on the board card is not built. Original brief: **Skip the opening titles and end credits — decided direction 2026-09-22, waiting on a second
...
31. ~~**Auto-posting through upload-post**~~ — done as code (ADR-0073, 2026-09-22), **not yet posted for real.** Port `ClipPublisher` (`submit` idempotent per our `request_id`, `status` incl. `NOT_FOUND`), `UploadPostPublisher` over `urllib` (multipart by hand, `async_upload`, `Idempotency-Key`), `InMemoryClipPublisher` stub. `ClipProduction(posting=)` publishes every `APPROVED` clip while the Run waits at `WAITING_HUMAN` (a `COMPLETED` Run may not own a non-terminal Task, `Run.restore`): one `publish-clip` Task, `request_id` = `<artifact_id>-publish`, **status first, submit only on `NOT_FOUND`**; success → `APPROVED → PUBLISHED` + URLs; failure → `PUBLISH_FAILED`, no auto-retry; no post text → `NO_POST_TEXT`. Tests `UPP`, `CRN-15` (incl. a crash after every commit → one submission per clip). `OMEMO_UPLOAD_POST_YOUTUBE_PRIVACY` is **required** once the key is set — the maintainer chose **`unlisted` to start** (2026-09-22), now in `.env` with `OMEMO_UPLOAD_POST_PLATFORMS="youtube"`; `demo_clips.py` builds with posting + post writer + desk. The real status route was probed: an unknown `request_id` → HTTP 404 `{"status":"not_found"}` → `NOT_FOUND`, as the adapter expects. Clip 1 of the 2026-09-22 re-run cannot be posted this way (its Run is `COMPLETED`, and it predates captions and post texts). Original brief: **Auto-posting through upload-post — chosen by the maintainer 2026-09-22; environment "без
