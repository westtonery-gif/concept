...
> Версия 1.2 (`ADR-0036`): субъект записи — Task **или** Evaluation (вызов QA-оценщика).
...
- **В scope.** Доменная сущность и её Value Objects, создание через Run, событие, ошибки; сбор
  ...
  через `ExecutionResult`; запись через `finish_task` (`ADR-0029`, ROADMAP Этап 7); запись вызовов
  QA-оценщика на Evaluation через `record_verdict` (`ADR-0036`, ROADMAP Этап 8).
- **Вне scope.** Агрегаты и отчёты; экспорт через Analytics Adapter; Analytics Agent; исход
  ...
  потому не имеющий достоверных usage/cost/model. Это не подменяется нулями. Счётчик попыток
  оценки (поэтому `retries` записи Evaluation — `None`).
...
- *(1.2)* QA-оценщик сообщает измерения тем же правилом: в `EvaluationResult.analytics` при
  вердикте и в `MeasuredEvaluatorError.analytics` при сбое после вызовов (`EVALUATION_SPEC.md` §8.3).
...
| `task_id` | `str \| None` | Run: переданная задача; `None` для записи Evaluation |
| `evaluation_id` | `str \| None` *(1.2)* | Run: переданная оценка; `None` для записи Task |
| `agent_ref` | `str` | Run, **из Task** или **из `evaluator_ref` Evaluation** |
...
| `retries` | `int \| None` | Run, **из Task**: `attempt_count − 1` на момент записи; для Evaluation — `None` |
| `prompt_ref` | `str \| None` (непустой, если задан) | вызывающий, необязателен |

- **Ровно один субъект** *(1.2)*: задан ровно один из `task_id` / `evaluation_id`; `retries`
  задан тогда и только тогда, когда субъект — Task. Иначе `InvalidAnalyticsRecordError` при
  создании записи.
...
| `record_evaluation_analytics(evaluation_id, *, provider, model, token_usage, cost, time_range, by, prompt_ref=None)` *(1.2)* | Content Director | Evaluation принадлежит Run (иначе `KeyError`) и называет `evaluator_ref` (иначе `InvalidAnalyticsRecordError`) | новая запись с `task_id=None`, `retries=None`; `AnalyticsRecordCaptured`; возвращается id |
...
- **Иных ограничений по состоянию нет:** можно записать неуспешный вызов, а также вызов после
  того, как Task, Evaluation или Run стали терминальными, — состоявшийся вызов есть факт.
- **Task → записи 1:N** и **Evaluation → записи 1:N** без предела (одна попытка может делать
  несколько вызовов модели).
- Все проверки выполняются **до** изменения состояния: отказ ничего не записывает и не расходует
  id.
- Обновления и удаления нет (append-only). Машины состояний Run, Task и Evaluation не меняются.
...
- *(1.2)* `record_verdict` сначала записывает все измерения оценщика на Evaluation, затем вердикт;
  при `MeasuredEvaluatorError` записывает её измерения и пробрасывает ту же ошибку.

## 5. События

`AnalyticsRecordCaptured(run_id, record_id, task_id, agent_ref, evaluation_id=None)` — в единый
журнал Run; у записи Evaluation `task_id is None` и задан `evaluation_id`. Сами метрики читаются из
записи.
...
| Ошибка | База | Когда |
|---|---|---|
| `InvalidAnalyticsRecordError` | `AnalyticsDomainError` | неправдоподобное значение VO; пустые `provider`/`model`/`prompt_ref`; не ровно один субъект или `retries` не по субъекту; Task не запускалась; Evaluation без `evaluator_ref` |
| `UnauthorizedActorError` (Run) | `RunDomainError` | не Content Director |
| `KeyError` | — | Task или Evaluation не принадлежит Run |
