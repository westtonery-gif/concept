...
| ID | Критерий |
|---|---|
| S9A-01 | Готовый бриф на доске → `produce_brief` возвращает Run: `run_id` — переданный, `content_brief_ref` — ссылка брифа, `workflow_version_ref` — id Workflow; вход первой Task — ровно текст брифа, и он есть в User первого вызова Rin; при `passed` Run `WAITING_HUMAN` (Approval Gate, ADR-0044), сохранённая строка равна Run. |
| S9A-02 | На доске под брифом — ровно `queued`, `running`, `waiting_qa`, `waiting_human`, каждый один раз, с `run_id` Run; `shown_status` равен статусу Run; отказанных отчётов нет. После `APPROVED` человека следующий `produce_brief` не вызывает моделей, Run `COMPLETED`, на доске добавляется ровно `completed`. |
...
| S9A-05 | Процесс падает сразу после коммита `queued` (до первой Task) → перезапуск `produce_brief`: вход первой Task — текст брифа с доски, не пустая строка; Run `WAITING_HUMAN`, доска показывает `waiting_human`. Если к перезапуску бриф снят с готовности — `BriefIntakeError`, сохранённый Run не изменён, модели не вызваны, новых отчётов нет. |
| S9A-06 | Модель QA нарушает грамматику вердикта → `QaVerdictError` пробрасывается, доска показывает `waiting_qa` (Run сохранён в нём); перезапуск спрашивает QA снова, producer-роли не вызываются, доска показывает `waiting_human`. |
| S9A-07 | Доска отказывает во всех отчётах (`BriefBoardError`) → прогон всё равно доходит до `WAITING_HUMAN` с тем же снимком, что при доступной доске; отказы в `failed_reports`. Доска снова доступна → следующий `produce_brief` не вызывает моделей и показывает `waiting_human`. |
